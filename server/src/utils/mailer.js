...
const sendWelcomeEmail = async ({ name, email, password, role }) => {
  try {
    ...
    const htmlContent = `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 550px; margin: 0 auto; padding: 30px; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #ffffff; color: #1a202c; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
        <div style="text-align: center; margin-bottom: 25px;">
          ...
          <!-- BRAND NEW DYNAMIC FLOW LINE VECTOR LOGO -->
          ...
          <p style="font-size: 13px; color: #4a5568; margin-top: 0;">Your workspace credentials are ready</p>
        </div>
    ...
    const info = await transporter.sendMail(mailOptions); // Using sendMail consistently
    console.log(`Welcome credentials email sent successfully to ${email}. Message ID: ${info.messageId}`);
    return info;
  } catch (err) {
